...
class PontoApp {
  ...
  _cacheDOMElements() {
    console.log("[CacheDOM v1.3.11] Caching DOM Elements...");
    this.ui = {
  ...
  _setupStaticEventListeners() {
    ...
    if (this.ui.btnLoginPromptTrigger) {
      this.ui.btnLoginPromptTrigger.addEventListener('click', () => {
        ...
        this.ui.loginModal?.show(); // Tenta mostrar usando instância cacheada
      });
    ...
    if (this.ui.btnNovoFuncAdminArea) {
      this.ui.btnNovoFuncAdminArea.addEventListener('click', () => {
        ...
        this.prepareEmployeeForm(null);
        this.ui.employeeFormModal?.show(); // Tenta mostrar usando instância cacheada
      });
  ...
  // Configura listeners para elementos DENTRO de um modal específico (chamado sob demanda)
  _setupModalEventListeners(modalName) {
    // Implementação mantida da versão anterior (v1.3.10)
    // ... (busca elementos internos e adiciona listeners com flags listenerAttached) ...
    console.log(`[Listeners] Setting up listeners for ${modalName}...`);
    const modalElement = this.ui[modalName + 'Element']; // Pega elemento DOM
    if (!modalElement) { console.error(`[Listeners] Elemento DOM ${modalName}Element não encontrado.`); return; }
    ...
    if (modalName === 'loginModal') {
      ...
      const employeeForm = modalElement.querySelector('#employeeForm');
      ...
      modalElement.querySelectorAll('input, select').forEach(input => { if (!input.listenerAttached) { input.addEventListener('input', () => { if (employeeForm?.classList.contains('was-validated')) { this._validateEmployeeForm(); } }); input.listenerAttached = true; } });
    }
  ...
  _initSelect2() {
    ...
    if (targetSelect && targetSelect.length > 0 && typeof $.fn.select2 === 'function') {
      ...
      catch (error) { console.error("Erro ao inicializar Select2:", error); this.showAlert('warning', 'Erro seletor func.') }
  ...
  _updateView() {
    ...
    if (this.state.token && this.state.currentUser) {
      if (this.ui.navLinksOffcanvas) this.ui.navLinksOffcanvas.style.display = 'block'; else console.error("navLinksOffcanvas missing");
      if (this.ui.navLogoutOffcanvas) this.ui.navLogoutOffcanvas.style.display = 'block'; else console.error("navLogoutOffcanvas missing");
      if (this.ui.authArea) { this.ui.authArea.innerHTML = `<span class="navbar-text me-3 small text-white-50">Olá, ${this.state.currentUser.fullName.split(' ')[0]}</span>`; } else { console.error("authArea missing"); }
      if (this.state.currentUser.role === 'admin') {
        if (this.ui.navAdminLinksOffcanvas) this.ui.navAdminLinksOffcanvas.style.display = 'block'; else console.error("navAdminLinksOffcanvas missing");
        if (this.ui.navAdminSeparatorOffcanvas) this.ui.navAdminSeparatorOffcanvas.style.display = 'block'; else console.error("navAdminSeparatorOffcanvas missing");
      ...
      if (this.ui.authArea) { this.ui.authArea.innerHTML = `<button class="btn btn-primary btn-sm" id="btnLoginTrigger">Login</button>`; } // Adiciona botão com ID
      this.setView('login');
      setTimeout(() => this._setupDynamicEventListeners(), 0); // Adiciona listener para o botão
    }
  ...
  async handleLogin() {
    ...
    const loginModal = this.ui.loginModal; if (!loginModal) { console.error("Login Modal not initialized."); return; }
    const loginForm = this.ui.loginModalElement?.querySelector('#loginForm'); const btnSubmit = this.ui.loginModalElement?.querySelector('#btnLoginSubmit'); const loginError = this.ui.loginModalElement?.querySelector('#loginError'); if (!loginForm || !btnSubmit || !loginError) { console.error("Elementos internos login modal não encontrados."); return; }
    this._setupAllModalEventListeners(); // Garante listeners internos
    const email = loginForm.email.value; const password = loginForm.password.value; loginError.style.display = 'none'; if (!email || !password) { loginError.textContent = 'E-mail/senha obrigatórios.'; loginError.style.display = 'block'; return; }
