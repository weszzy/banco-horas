...
class BalanceService {
    ...
    calculateDailyBalance(timeRecord, employee) {
        ...
        if (!timeRecord || !employee || typeof timeRecord.totalHours !== 'number' || !employee.weeklyHours) {
            logger.warn(`[BalanceService] Dados incompletos para calculateDailyBalance. Rec ID: ${timeRecord?.id}, Emp ID: ${employee?.id}`);
            return null;
        }
        try {
            // Obtém a meta diária com base na carga horária do funcionário
            const dailyGoal = this._calculateDailyGoal(parseFloat(employee.weeklyHours));
            // Obtém as horas trabalhadas calculadas pelo hook do TimeRecord
            const workedHours = parseFloat(timeRecord.totalHours);

            // Valida se os números são válidos
            if (isNaN(dailyGoal) || isNaN(workedHours)) {
                logger.warn(`[BalanceService] Meta diária ou horas trabalhadas inválidas (NaN) em calculateDailyBalance. Rec ID: ${timeRecord.id}`);
                return null;
            }

            // Calcula o saldo: horas trabalhadas - meta diária
